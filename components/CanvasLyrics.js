...
class CanvasLyrics {
  constructor (config, callbacks, player) {
    ...
    this.askDevices = callbacks.askDevices;
    this.alert = callbacks.alert;
    this.command = callbacks.command;
    ...
    console.log("[SPOTIFY][CanvasLyrics] Class Loaded");
  }
  ...
  prepare () {
    ...
    logo.addEventListener("click", () => {
      if (this.player.deviceName !== this.currentPlayback.device.name) {
        this.closeDisplay();
      }
    });
    ...
    speakerName.textContent= "@bugsounet Coding";
    ...
    controlPlay.addEventListener("click", () => {
      if (this.currentPlayback && this.currentPlayback.is_playing) this.command("PAUSE");
    ...
    volumeValue.addEventListener("click", (e) => {
      var x = e.offsetX;
      var clickedValue = x * 100 / (e.target.offsetWidth);
      this.command("VOLUME", clickedValue.toFixed(0));
    });
    ...
    /** Draw SVG icon **/
    var drawSpotify = SVG().addTo("#EXT_SPOTIFYCL_LOGO").viewbox(0,0,1134,340).fill("#1db954")
      .move(0, 10);
    drawSpotify = drawSpotify.path(this.paths.spotify);

    var drawShuffle = SVG().addTo("#EXT_SPOTIFYCL_SHUFFLE").size("32px", "32px").viewbox(0,0,16,16)
      .fill("white")
      .move(0, 10)
      .addClass("shuffleCL");
    ...
    var drawPrevious = SVG().addTo("#EXT_SPOTIFYCL_PREVIOUS").size("32px", "32px").viewbox(0,0,16,16)
      .fill("white")
      .move(0, 10);
    drawPrevious = drawPrevious.path(this.paths.previous);

    var drawPlay = SVG().addTo("#EXT_SPOTIFYCL_PLAY").size("32px", "32px").viewbox(0,0,16,16)
      .fill("black")
      .move(0, 10)
      .addClass("playCL");
    drawPlay = drawPlay.path(this.paths.play);

    var drawNext = SVG().addTo("#EXT_SPOTIFYCL_NEXT").size("32px", "32px").viewbox(0,0,16,16)
      .fill("white")
      .move(0, 10);
    drawNext = drawNext.path(this.paths.next);

    var drawRepeat = SVG().addTo("#EXT_SPOTIFYCL_REPEAT").size("32px", "32px").viewbox(0,0,16,16)
      .fill("white")
      .move(0, 10)
      .addClass("repeatCL");
    drawRepeat = drawRepeat.path(this.paths.repeat);

    var drawVolume = SVG().addTo("#EXT_SPOTIFYCL_VOLUMEICON").size("32px", "32px").viewbox(0,0,16,16)
      .fill("white")
      .move(0, 10)
      .addClass("volumeCL");
    ...
    var speaker = SVG().addTo("#EXT_SPOTIFYCL_SPEAKERICON").size("32px", "32px").viewbox(0,0,16,16)
      .fill("#1db954")
      .move(0, 10);
    ...
    this.devicesPanel = new JSPanel(buttonSpeaker, {
      bottom: 40,
      right: 0,
      left: 0,
      items: [
  ...
  decodeLyrics (result) {
    return new Promise((resolve) => {
      ...
      if (error || result.lyrics.syncType === "UNSYNCED") {
        ...
        console.log("[SPOTIFY][Lyrics] Lyrics not sync with title");
      }
      resolve(lyrics);
    });
  }
  ...
  displayLyrics () {
    console.log("[SPOTIFY][Lyrics] Display Lyrics");
